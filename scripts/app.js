...
function countTotalIngredients(recipesList, index = 0) {
  if (index >= recipesList.length) return 0;
  return recipesList[index].ingredients.length + countTotalIngredients(recipesList, index + 1);
}
...
function showUniqueIngredients() {
  const allIngredients = recipes.flatMap(r => r.ingredients);
  const unique = _.uniq(allIngredients);
  console.log("Unique ingredients (Lodash):", unique);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const totalIngredients = countTotalIngredients(recipes);
  console.log("Total ingredients across all recipes (recursive):", totalIngredients);
